...
# Supabase-py needs my Supabase URL and key to connect to my project. You can find the URL in the supabase dashboard overview page righ under the project name.
# The key can be found within the setting page under the API keys section. This key should never be hardcoded into the code because it
# can be exploited by anyone who comes across the key.

# Connection Q2

def get_client():           #loads supabse URL and key from .env file and creates a supabase client
...
def insert_test_record(supabase):
    ...
    response = supabase.table("weather_raw").upsert(record).execute()
    return response
...
rows = get_records_by_date_range(supabase, "2026-09-01", "2026-09-30")
print(rows)
...
# Idompotency is important in web applications because it ensures that a request can be made 
